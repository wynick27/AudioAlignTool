...
- 语言感知分句可处理中文、日文和空格语言，支持省略号、多重标点、直接引语及含多个逗号的长句，不会产生只有标点的句段。
...
- ASR 对比视图分栏显示原文与识别稿、恢复空格语言的词间空格并同步滚动；点击识别词会跳到其词级时间并将主音频视图居中。
...
- “原书”使用 WebEngine 显示 EPUB/HTML 的原始 XHTML、CSS、字体、图片和排版；TXT、Markdown、SRT 与纯音频项目使用阅读模板。
- 原书文字点击后按播放状态决定是仅定位还是继续播放；播放时高亮实际文字范围并可自动滚动，同时保留浏览器原生选择、复制与缩放。
...
- 支持交互式 HTML、SRT、WebVTT、schema v2 JSON，以及 EPUB 3 Media Overlays。
