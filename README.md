...
- PySide6 桌面界面；字幕表格、连续文章音频、ASR 对比和保留来源样式的“原书”四种视图。
...
- TXT、Markdown、HTML、EPUB、SRT，以及 MP3、M4A、M4B、AAC、WAV、FLAC、OGG、Opus 和含音轨的 MP4/MKV/MOV/WebM。M4B 内嵌章节作为同一资源中的时间切片参与配对。
- TXT/MD/HTML/SRT 文件选择器支持多选；多选时默认每个文件建立一个章节。单个 MD/HTML 仍可按一级标题拆章。
- EPUB 中只有一句的 `CHAPTER XXX` 标题页会与下一正文页组成一个逻辑章节，但标题句本身作为可朗读正文完整保留；HTML `<head><title>` 等不可见元数据不会进入正文。
- 媒体资源与章节配对管理器：添加、排序、重命名、重新定位、替换、移除引用、检查重复项、多切片配对和一次事务应用。导入媒体不会擅自创建章节或自动占用章节。
- 同一媒体切片可复用到多个文本章节；识别缓存按章节隔离，避免相同音频范围串用其他章节的匹配结果。
...
- HTML、SRT、WebVTT、schema v2 JSON，以及保留原 EPUB 排版资源的 EPUB 3 Media Overlays 导出。
- EPUB 导出遇到个别无法映射的句段时会跳过并列出警告，其余章节继续生成；EPUB/OPF 结构或媒体不可用仍会停止导出。
- 撤销/重做按“数据库规范路径＋项目＋会话＋章节”隔离，并原子恢复句段和文字—音频锚点；项目切换会取消旧任务并清空旧页面、播放范围和历史。
