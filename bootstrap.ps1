...
if (-not (Test-Path $python)) {
    ...
    if ($null -ne $launcher) {
        ...
        if (-not (Test-Path -LiteralPath $installedPython)) {
            throw "Python 3.14 was not found. Install it or create .venv manually."
        }
        & $installedPython -m venv (Join-Path $root ".venv")
    }
}
& $python -c "import sys; assert sys.version_info[:2] == (3, 14), f'Python 3.14 is required; found {sys.version}'"
if ($LASTEXITCODE -ne 0) {
    throw "The existing .venv is not Python 3.14. Remove .venv and run bootstrap.ps1 again."
}
