...
def _annotate_html_export_page(
    ...
    script.string = r"""
      ...
      addEventListener('message',function(event){
        ...
        const target=document.querySelector('[data-aat-index="'+event.data.index+'"]');
        if(target){target.classList.add('aat-active');if(event.data.follow)target.scrollIntoView({block:'center',behavior:'smooth'});}
      });
    """
...
const data=JSON.parse(document.getElementById('alignment').textContent),select=document.getElementById('chapters'),player=document.getElementById('player'),book=document.getElementById('book'),clock=document.getElementById('clock'),speed=document.getElementById('speed'),loop=document.getElementById('loop');let active=-1,segments=[],links=[],linkIndex=0,lastSaved=0;const storageKey='AudioAlignTool:'+data.project_id;
